...
class Agent:
    def __init__(self, discount_factor=.95, buffer_size=600, k=1000, load=False, load_file="qModelHigherLR.pt", save_file ="qModelHigherLRheuristic.pt", epsilon=1, epsilon_decay=.995, epsilon_min=.05, distributed=False, device=torch.device("cpu")):
        ...
        self.k = k
        self.i = 0
        self.epsilon = epsilon if not load else .25
        ...
        self.optim = torch.optim.AdamW(self.q_model.parameters(), lr=.001)
    ...
    def replay(self, batch_size):
        ...
        if self.i % 200 == 0:
            ...
            if self.epsilon > self.epsilon_min:
                ...
                self.epsilon = self.epsilon if self.epsilon >= self.epsilon_min else self.epsilon_min
        if self.i % self.k == 0:
            if self.distributed:
